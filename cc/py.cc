...
PYBIND11_MODULE(hidb_backend, m)
{
    ...
    py::class_<HiDb>(m, "HiDb")
            ...
            .def("export_to", &HiDb::exportTo, py::arg("filename"), py::arg("pretty") = false, py::arg("timer") = false)
            .def("import_from", &HiDb::importFrom, py::arg("filename"), py::arg("timer") = false)
            .def("import_locdb", &HiDb::importLocDb, py::arg("filename"), py::arg("timer") = false)
